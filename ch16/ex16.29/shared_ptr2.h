...
public:
    ...
    shared_ptr2(T * tp, std::function<void (T *)> fp = nullptr)
        : p(tp), ip(new int(1)), delFun(fp) {  }
    ~shared_ptr2();
...
template <typename T>
shared_ptr2<T>::shared_ptr2(const shared_ptr2 & sp, std::function<void (T *)> fp)
    : p(sp.p), ip(sp.ip)
{
    ++*ip;
    if(fp == nullptr)
...
template <typename T>
shared_ptr2<T>::shared_ptr2(shared_ptr2 && sp, std::function<void (T *)> fp)
    : p(std::move(sp.p)), ip(std::move(sp.ip))
{
    if(fp == nullptr)
        delFun = sp.delFun;
    else
