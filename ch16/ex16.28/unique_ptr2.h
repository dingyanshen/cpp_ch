...
{
public:
    unique_ptr2() : p(nullptr), delFun(Unique_ptr_Delete()) { }
    unique_ptr2(T * up, F fun = Unique_ptr_Delete()) : p(up) , delFun(fun) { }
    unique_ptr2(F fun) : p(nullptr) , delFun(fun) { }
    ~unique_ptr2();
...
template <typename T, typename F>
unique_ptr2<T, F>::unique_ptr2(unique_ptr2 && up)
{
    p = std::move(up.p);
